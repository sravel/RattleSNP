...
import click
import rattleSNP
from rattleSNP.snakeWrapper import *
...
def main(ctx, restore):
    if ctx.invoked_subcommand is None and restore and check_privileges():
        if INSTALL_MODE.exists():
            ...
            click.secho(f"\n    Remove installation mode, now run:\n    rattleSNP install_local or install_cluster\n\n", fg="yellow")
        else:
            click.secho(f"\n    No reset need, rattleSNP not install !!!!!\n    Please run: rattleSNP install_local or install_cluster !!!!\n\n", fg="red")
    pass
...
if "sphinx" in args:
    ...
    main.add_command(rattleSNP.edit_cluster_config)
