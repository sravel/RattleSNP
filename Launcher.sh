...
#snakemake --configfile $@ --cluster-config ${cluster_config} --rulegraph  > schema_pipeline_global.dot && dot schema_pipeline_global.dot -Tpdf > schema_pipeline_global.pdf
#exit
...
snakemake --latency-wait 5184000 --jobs 100 --cluster "qsub {cluster.queue} {cluster.export_env} {cluster.cwd} {cluster.mem} {cluster.n_cpu} {cluster.logerror} {cluster.log} " --cluster-config ${cluster_config}  --configfile $@
